...
class SNumericPropertyWidget : public SCompoundWidget
{
public:

	SLATE_BEGIN_ARGS( SNumericPropertyWidget<NumericType> )
	...
	void Construct( const FArguments& InArgs)
	{
		...
		else
		{
			...
			[
				...
				.Value(this, &SNumericPropertyWidget<NumericType>::OnGetValue)
				.Font(InArgs._Font)
				.MinValue(MinValue)
				.MaxValue(MaxValue)
				...
				.OnValueChanged(this, &SNumericPropertyWidget<NumericType>::OnValueChanged)
				.OnValueCommitted(this, &SNumericPropertyWidget<NumericType>::OnValueCommitted)
				.OnUndeterminedValueCommitted(this, &SNumericPropertyWidget<NumericType>::OnUndeterminedValueCommitted)
				.OnBeginSliderMovement(this, &SNumericPropertyWidget<NumericType>::OnBeginSliderMovement)
				.OnEndSliderMovement(this, &SNumericPropertyWidget<NumericType>::OnEndSliderMovement)
	...
	void GetDesiredWidth( float& OutMinDesiredWidth, float& OutMaxDesiredWidth )
	{
		...
		const bool bIsNonEnumByte = (Property->IsA(FByteProperty::StaticClass()) && Cast<const FByteProperty>(Property)->Enum == NULL);
...
private:
	...
	{	
		static bool Match(const FProperty* InProperty) { return InProperty->IsA(UFloatProperty::StaticClass()); } 
	};

	template<typename U>
	struct TTypeToProperty<double, U>
	{
		static bool Match(const FProperty* InProperty) { return InProperty->IsA(UDoubleProperty::StaticClass()); }
	};

	template<typename U>
	struct TTypeToProperty<int8, U>
	{
		static bool Match(const FProperty* InProperty) { return InProperty->IsA(UInt8Property::StaticClass()); }
	};

	template<typename U>
	struct TTypeToProperty<int16, U>
	{
		static bool Match(const FProperty* InProperty) { return InProperty->IsA(UInt16Property::StaticClass()); }
	};

	template<typename U>
	struct TTypeToProperty<int32, U>
	{
		static bool Match(const FProperty* InProperty) { return InProperty->IsA(UIntProperty::StaticClass()); }
	};

	template<typename U>
	struct TTypeToProperty<int64, U>
	{
		static bool Match(const FProperty* InProperty) { return InProperty->IsA(UInt64Property::StaticClass()); }
	};

	template<typename U>
	struct TTypeToProperty<uint8, U>
	{
		static bool Match(const FProperty* InProperty) { return (InProperty->IsA(FByteProperty::StaticClass()) && Cast<const FByteProperty>(InProperty)->Enum == NULL); }
	};

	template<typename U>
	struct TTypeToProperty<uint16, U>
	{
		static bool Match(const FProperty* InProperty) { return InProperty->IsA(UUInt16Property::StaticClass()); }
	};

	template<typename U>
	struct TTypeToProperty<uint32, U>
	{
		static bool Match(const FProperty* InProperty) { return InProperty->IsA(UUInt32Property::StaticClass()); }
	};

	template<typename U>
	struct TTypeToProperty<uint64, U>
	{
		static bool Match(const FProperty* InProperty) { return InProperty->IsA(UUInt64Property::StaticClass()); }
	};
